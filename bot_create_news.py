...
async def confirm_selected_topics(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    ...
    # Вызываем get_topics, чтобы получить список всех тем
    topics_data = await get_topics(dialog_manager)
    all_topics = topics_data["topics"]  # Список тем в формате [("IT", '1'), ("Дизайн", '2'), ...]

    # Отбираем названия тем по их идентификаторам
    ...
    # Очищаем стек и возвращаемся в стартовое меню с использованием show_mode=ShowMode.SEND
    await dialog_manager.switch_to(state=MainDialogSG.start, show_mode=ShowMode.SEND)



# Перший діалог (меню підписок і вибір тем)
start_dialog = Dialog(
    Window(
